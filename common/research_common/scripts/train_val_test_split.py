...
def _check_for_previous_split(dataset: ROCODataset):
    for split in Split:
...
def _save_set_split(dataset: ROCODataset, split: Split, file_paths: Iterable[Path]):
    split.get_split_file(dataset).write_text("\n".join(file.stem for file in file_paths))


def _create_splits(dataset: ROCODataset):
...
def train_test_split_set(dataset: ROCODataset):
